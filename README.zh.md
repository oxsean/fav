...
`Ctrl+N/P` 上下 / `Ctrl+G` 同 `Space`；大写 `D M X` 输入法不拦；操作框里所有动作都有按钮。`?` 看全部键位（按在哪里生效分组）。
...
`after:2026-09-01`、`before:…`（按会话开始时间）、`last:7d` / `last:2026-09-01`（按最近活动：上周开始、今天还在用的也算）、`turns:3`、`file:internal/index`（AI 写过路径含这段的文件），以及普通关键词。全部 AND，中文直接子串匹配。
